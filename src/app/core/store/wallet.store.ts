...
export class WalletStore {
  ...
  private readonly _topFundedLoading = signal(false);
  private readonly _topFundedError = signal<string | null>(null);
  topFundedLoading = this._topFundedLoading.asReadonly();
  topFundedError = this._topFundedError.asReadonly();
...
fetchMetrics(params: { custom_range?: string; start_date?: string; end_date?: string } = {}): void {
  ...
  this.walletService.getWalletMetrics(params).subscribe({
    next: (res) => {
  ...
  this._topFundedLoading.set(true);
  this._topFundedError.set(null);

  this.walletService.getTopFundedWallets().subscribe({
    next: (res) => {
      const mapped: TopFundedWallet[] = res.data.top_wallet_balances.map((w) => ({
        id:               w.wallet_id,
        rank:             w.rank,
        balance:          w.balance,
      ...
      this._topFundedWallets.set(mapped);
      this._topFundedLoading.set(false);
    },
    error: (err: any) => {
      this._topFundedError.set(err?.error?.message ?? 'Failed to load top funded wallets.');
      this._topFundedLoading.set(false);
    },
  });
}
